...
public abstract class AbstractPersistenceManager {
    ...
    protected final AmazonDynamoDB amazonDynamoDbClient;

    public AbstractPersistenceManager(final AmazonDynamoDB amazonDynamoDbClient, final int readThroughput,
    ...
    protected final void initSchema(final DatabaseSchemaHolder databaseSchemaHolder) {
        ...
        for (final ItemConfiguration itemConfiguration : databaseSchemaHolder.itemConfigurations()) {
            ...
            if (!tablesPendingCreation.contains(tableName) && !isTableCreated(tableName)) {
                ...
                if (CompoundPrimaryKeyDefinition.class.isAssignableFrom(primaryKeyDefinition.getClass())) {
                    ...
                    attributeDefinitions.add(new AttributeDefinition().withAttributeName(rangeKey).withAttributeType(
                            rangeKeyType));
                ...
                for (final IndexDefinition indexDefinition : itemConfiguration.indexDefinitions()) {
                    ...
                    attributeDefinitions.add(new AttributeDefinition()
                            .withAttributeName(indexDefinition.propertyName()).withAttributeType(attributeType));
                ...
                final ProvisionedThroughput tableProvisionedThroughput = new ProvisionedThroughput()
                        .withReadCapacityUnits((long) readThroughput).withWriteCapacityUnits((long) writeThroughput);
            ...
            if (!itemConfiguration.uniqueConstraints().isEmpty()) {
                ...
                if (!isTableCreated(uniqueConstraintTableName)) {
                    ...
                    final CreateTableRequest createTableRequest = new CreateTableRequest()
                            .withTableName(uniqueConstraintTableName).withAttributeDefinitions(attributeDefinitions)
                            .withKeySchema(keySchema).withProvisionedThroughput(tableProvisionedThroughput);
        ...
        if (tablesPendingCreation.size() != 0) {
            throw new IllegalStateException("Unable to create tables for DynamoDb template: " + tablesPendingCreation);
        }
