...
/**
 * In-memory implementation of {@link TypedMessageListener}
 */
public class InMemoryTypedMessageListener extends InMemoryMessageListener<TypedMessage> implements TypedMessageListener {

    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final Map<String, MessageHandler<TypedMessage>> messageHandlers = new HashMap<>();

    public InMemoryTypedMessageListener(final MessageQueue<TypedMessage> typedMessageQueue,
            final InMemoryMessageQueuePoller inMemoryMessageQueuePoller) {
        super(typedMessageQueue, inMemoryMessageQueuePoller);
    }
